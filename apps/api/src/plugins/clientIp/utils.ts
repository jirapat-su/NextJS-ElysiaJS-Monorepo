 ...
 */
const getHeader = (headers: HeaderSource, key: string): string | undefined => {
...
const extractPublicIp = (headers: HeaderSource): string | undefined => {
...
const extractAnyValidIp = (headers: HeaderSource): string | undefined => {
  for (const headerName of IP_HEADERS) {
    const value = getHeader(headers, headerName);
    if (!value) {
      continue;
 ...
 */
export const extractClientIpAddress = (
 ...
 * - Converts IPv4-mapped IPv6 (::ffff:127.0.0.1) to IPv4
